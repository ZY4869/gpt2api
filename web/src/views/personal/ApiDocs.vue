...
resp = client.chat.completions.create(
    model="${model}",
    messages=[{"role": "user", "content": "请画一张雨夜霓虹街头的赛博朋克海报"}],
    extra_body={"image_generation": True},
)
print(resp.model_extra["images"][0]["url"])`
})
...
const responsesJSONExample = computed(() => {
  return `{
  "id": "resp_xxx",
  "object": "response",
  ...
  "output": [
    ...
    },
    {
      ...
      "result": [
        {
          "type": "output_image",
          "url": "${origin.value}/p/img/img_xxx/0?exp=...&sig=...",
          "file_id": "file-xxx",
      ...
      <div>
        ...
        <p class="desc">
          <template v-if="ENABLE_CHAT_MODEL">
            ...
            <template v-if="ENABLE_CHAT_IMAGE_MIXED">, 以及 <code>/v1/responses</code></template>,
          </template>
          <template v-else>
    ...
    <el-tabs v-model="activeTab" class="pg-tabs">
      ...
      <el-tab-pane v-if="ENABLE_CHAT_MODEL" label="对话生成(文字模型)" name="chat">
        <div class="card-block">
          ...
          <template v-if="ENABLE_CHAT_IMAGE_MIXED">
            <el-alert
              type="warning"
              :closable="false"
              show-icon
            ...
            <el-tabs type="border-card" class="code-tabs" style="margin-top: 12px">
              <el-tab-pane label="Chat + 生图(curl)">
                <pre class="code"><code>{{ chatImageCurl }}</code></pre>
                <el-button size="small" @click="copy(chatImageCurl)">复制 curl</el-button>
              </el-tab-pane>
              <el-tab-pane label="Chat + 生图(Python)">
                <pre class="code"><code>{{ chatImagePython }}</code></pre>
                <el-button size="small" @click="copy(chatImagePython)">复制 Python</el-button>
              </el-tab-pane>
              <el-tab-pane label="Responses(curl)">
