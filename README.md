...
[![Release](https://img.shields.io/badge/release-v0.5.34-brightgreen?style=flat-square)](https://github.com/432539/gpt2api/releases)
...
> 当前默认版本：`v0.5.34`，作为当前 fork 的发布线；上游 `v2.x` 仍保留为新架构参考基线。
...
- 修复 GPT Web 套图回收过早结束的问题：不足额结果必须等到上游 Conversation 明确完成后才允许收口
- 请求 10 张但上游正常完成只生成 8 张时，按实际 8 张成功返回，不再把“部分稳定”误判为正式结束
- 图片任务最终结算改为按实际返回张数退差额，并更新任务最终成本
- 新增 provider / service 层测试，覆盖完成信号识别、部分套图收口与实际张数成本计算
