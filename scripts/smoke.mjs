...
// ========== 10. mixed-mode(可选) ==========
async function checkMixedMode() {
  step('10. mixed-mode 对话框生图(可选)')
  if (!ENABLE_MIXED) {
    skip('mixed-mode 冒烟组', '未启用 --mixed-mode=true / GPT2API_SMOKE_MIXED=true')
    return
  }

  const create = await call('POST', '/api/keys', { token: userToken, body: {
    name: 'smoke-mixed-key', quota_limit: 0, rpm: 60, tpm: 60000,
  }})
  const cd = unwrap(create)
  if (create.status !== 200 || !cd?.key?.startsWith?.('sk-')) {
    bad('创建 mixed-mode Key 失败', err(create))
    return
  }
  ok(`创建 mixed-mode Key 成功, id=${cd.record.id}`)

  const apiKey = cd.key
  const cleanup = async () => {
    if (KEEP_USER) return
    await call('DELETE', `/api/keys/${cd.record.id}`, { token: userToken }).catch(() => {})
  }

  const chatStream = await call('POST', '/v1/chat/completions', { token: apiKey, body: {
    model: MIXED_MODEL,
  ...
  const chatResp = await call('POST', '/v1/chat/completions', { token: apiKey, body: {
    model: MIXED_MODEL,
    image_generation: true,
    stream: false,
    messages: [{ role: 'user', content: MIXED_PROMPT }],
  }})
  if (chatResp.status === 403 && openaiCode(chatResp) === 'feature_disabled') {
    ok('mixed-mode feature flag 关闭时, /v1/chat/completions 返回 403 feature_disabled')
    await cleanup()
    return
  }
  if (shouldSkipMixedMode(chatResp)) {
    skip('chat mixed-mode 成功链路', `环境未满足:${openaiCode(chatResp)} ${openaiMsg(chatResp)}`)
    await cleanup()
    return
  }
  ...
  if (shouldSkipMixedMode(responsesResp)) {
    skip('responses mixed-mode 成功链路', `环境未满足:${openaiCode(responsesResp)} ${openaiMsg(responsesResp)}`)
    await cleanup()
    return
  }
