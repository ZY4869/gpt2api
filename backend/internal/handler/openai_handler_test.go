...
func TestOpenAIResultBaseURLPrefersForwardedHost(t *testing.T) {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	req := httptest.NewRequest("GET", "http://openai/v1/files/task_123/0", nil)
	req.Host = "140.245.121.220"
	req.Header.Set("X-Forwarded-Proto", "http")
	req.Header.Set("X-Forwarded-Host", "140.245.121.220:17200")
	c.Request = req

	got := openAIResultBaseURL(c)
	want := "http://140.245.121.220:17200"
	if got != want {
		t.Fatalf("expected %q, got %q", want, got)
	}
}
