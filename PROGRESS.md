...
### GPT Web 套图完成信号与实际张数结算

- 修复 `wait_all_then_download` / Web 多图回收在只拿到 1 张或部分图片稳定后就提前成功的问题
- 新增上游 Conversation 完成信号识别：只有明确正常完成后，才允许不足请求数量的套图按实际张数返回
- 保留完整套图的稳定顺序回收逻辑；未见上游完成信号时，部分结果不会被当作正式成功
- 图片任务成功结算改为按实际返回张数退差额，并同步更新任务最终成本
- 补充 provider / service 单测，覆盖 running 不收尾、finished_successfully 不足额收尾、SSE completed 不误判和实际张数成本计算

---

## v0.5.33 — 2026-05-09

### GPT Web 套图测试模式超时与部分返回优化

- 优化 `wait_all_then_download` 测试模式，最长等待时间从 `30 分钟` 缩短到 `10 分钟`
- 当上游超时或提前停产时，允许直接返回当前已回收到的图片，不再因 `8/10` 这类不满套结果整单失败
- 增加“部分结果稳定”智能收口：若候选集合顺序连续稳定若干轮，则自动提前结束并回收现有图片
- 同步更新任务级超时与 stale 判定逻辑，并补充 provider / service 层测试
...
- 修复 GPT Web 图片下载链路在遇到 `download_url` 包装 JSON 时误把元信息缓存成本地结果文件的问题
- 现在会继续追包装里的下一跳图片地址，直到拿到真实图片字节，避免 `/v1/files/:task_id/:seq` 返回 412B 伪 `.png`
- 新增 provider 单测，覆盖包装下载地址提取与相对下载地址归一化，继续锁定 10 张套图的真实落盘链路
...
- 修复 `wait_all_then_download` 严格模式轮询退出条件过严的问题，避免 `10/10` 候选图已收齐时仍长期卡在 `web.poll.conversation / web.poll.resolve`
- 继续保留“必须整套收齐再统一下载”的硬约束，但当 authoritative 顺序迟迟不稳定时，允许按“当前稳定顺序”或“首次完整顺序”进入最终统一下载
- 新增 provider 单测，覆盖严格模式三条关键退出路径：不足整套不退出、当前顺序稳定可退出、首次完整顺序在额外等待后可退出
